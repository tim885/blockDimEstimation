# transfer learning script for block dimension estimation
...
parser = argparse.ArgumentParser(description='Pytorch transfer learning for block dimension estmation')
...
parser.add_argument('--numClass', default=[10, 10, 10], type=list,
...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        err_x_train, err_y_train, err_z_train, loss = train(train_loader, model, criterion, optimizer, epoch)

        # evaluate on validation set after training of one epoch
        err_x_val, err_y_val, err_z_val, prec1, conf_x, conf_y, conf_theta = validate(val_loader, model, criterion)

        # remember best prec1 and save checkpoint
        is_best = prec1 > best_prec1
        best_prec1 = max(prec1, best_prec1)
        losses[epoch] = np.array(loss)
        errors[epoch, :] = [err_x_train, err_x_val, err_y_train, err_y_val, err_z_train, err_z_val]
        save_checkpoint({
            'epoch': epoch + 1,
            'arch': args.arch,
            ...
            'errors': errors,
            'date_time': date_time,
        }, is_best, args.results_path+'checkpoint.pth.tar')

        # save classification error/loss
        if epoch == 0:
            f_error = open(args.results_path+'error_'+date_time+'.log', 'w')
            f_error.write('Epoch Training_error_x Training_error_y Training_error_z'
                          'Validation_error_x Validation_error_y Validation_error_z')
            f_loss = open(args.results_path+'loss_' + date_time + '.log', 'w')
            f_loss.write('Epoch Training_loss')
        else:
            f_error = open(args.results_path+'error_'+date_time+'.log', 'a')
            f_loss = open(args.results_path+'loss_' + date_time + '.log', 'a')
        ...
        fig_loss.savefig(args.results_path+'fig_train_loss_'+date_time+'.log.eps')
        ...
        plt.legend(('train_err_x', 'val_err_x', 'train_err_y', 'val_err_y', 'train_err_z', 'val_err_z'),
                   loc=(0.01, 0.01))
        plt.xlabel('epoch'); plt.ylabel('train/val errors')
        plt.title('Training/Validation errors - epoch')
        fig_err.savefig(args.results_path+'fig_errors_' + date_time + '.log.eps')
        ...
        if (epoch+1) % 5 == 1:
            ...
            fig_conf_x.savefig(args.results_path+'fig_confusion_x_' + str(epoch + 1) + '.jpg')
            ...
            fig_conf_y.savefig(args.results_path+'fig_confusion_y_' + str(epoch + 1) + '.jpg')
            ...
            plt.xlabel('predicted class'); plt.ylabel('actual class')
            plt.title('classification along z')
            fig_conf_theta.savefig(args.results_path+'fig_confusion_z_' + str(epoch + 1) + '.jpg')
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        loss_z = criterion(output_z, target_z)
        global_loss = loss_x + loss_y + loss_z

        # measure average accuracy and record loss
        prec1_x, prec1_y, prec1_z, prec5_x, prec5_y, prec5_z = \
            accuracy(output, target, topk=(1, 5))
        prec1 = (prec1_x + prec1_y + prec1_z)/3
        prec5 = (prec5_x + prec5_y + prec5_z)/3
        err1_x = 100 - prec1_x
        err1_y = 100 - prec1_y
        err1_z = 100 - prec1_z
        ...
        err_z.update(err1_z[0], input.size(0))
    ...
    return [err_x.avg, err_y.avg, err_z.avg, losses.avg]
...
def validate(val_loader, model, criterion):
    ...
    with torch.no_grad():
        ...
        for i, (input, target) in enumerate(val_loader):  # for each mini-batch
            ...
            output_x, output_y, output_z = model(input)
            output = [output_x, output_y, output_z]
            outputs = torch.cat((outputs, torch.cat((output_x, output_y, output_z), 1)), 0)
    ...
    confusion_x, confusion_y, confusion_z = gen_confusion(outputs, targets)

    return [err_x.avg, err_y.avg, err_z.avg, top1.avg, confusion_x, confusion_y, confusion_z]
...
def save_checkpoint(state, is_best, filename):
    ...
    if is_best:  # store model with best perf
        shutil.copyfile(filename, args.results_path+'model_best.path.tar')
...
def accuracy(output, target, topk=(1,)):
    ...
    with torch.no_grad():  # no grad computation to reduce memory
        ...
        _, pred_z = output_z.topk(maxk, 1, True, True)
        ...
        correct_z = pred_z.eq(target_z.view(1, -1).expand_as(pred_z))
        ...
        for k in topk:  # store in res and output
            ...
            correct_k_z = correct_z[:k].view(-1).float().sum(0, keepdim=True)

            res.append(correct_k_x.mul_(100 / batch_size))
            res.append(correct_k_y.mul_(100 / batch_size))
            res.append(correct_k_z.mul_(100 / batch_size))
...
def gen_confusion(outputs, targets):
    ...
    confusion_z = torch.zeros(args.numClass[2], args.numClass[2])
    ...
    for i in range(0, targets.size(0)):  # each row represents a ground-truth class
        ...
        confusion_z[target_z[i], pred_z[i]] += 1
    ...
    for i in range(0, args.numClass[2]):
        if confusion_z[i].sum() == 0:
            confusion_z[i] = 0
        else:
            confusion_z[i] = confusion_z[i] / confusion_z[i].sum()

    return [confusion_x, confusion_y, confusion_z]
...
class BlockDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img_path = args.dataset_path + img_path
